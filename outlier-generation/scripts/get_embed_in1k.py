...
parser.add_argument('--inlier', type=int, default=False)
...
anchor = torch.from_numpy(np.load('./token_embed_in1k.npy')).cuda()
num_classes = 1000
sum_temp = 0


data_dict = torch.from_numpy(np.load('./id_feat_in1k_99epoch.npy')).cuda()
for index in range(1000):
    sum_temp += 1000  # number_dict[index]
# breakpoint()
if sum_temp == num_classes * 1000:
