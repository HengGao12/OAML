...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default="/home1/gaoheng/gh_workspace/dream-ood-main/nobackup-slow/dataset/my_xfdu/diffusion/sd-v1-4.ckpt",
        help="path to checkpoint of model",
    )
    parser.add_argument(
        "--seed",
    ...
    # parser.add_argument("--ckpt", type=str, default="/nobackup-slow/dataset/my_xfdu/diffusion/sd-v1-4.ckpt", help="path to checkpoint of model")
    # opt = parser.parse_args()
    ...
    # 
    # model = instantiate_from_config(config, opt.ckpt)

    device = torch.device("cuda:5") if torch.cuda.is_available() else torch.device("cpu")
    model = model.cuda()
    # fine_labels = ['stingray', 'hen', 'magpie', 'kite', 'vulture', 'agama', 'tick', 'quail', 'hummingbird', 'koala']
    ...
    'stinkhorn mushroom', 'earth star fungus', 'hen of the woods mushroom',
    'bolete', 'corn cob', 'toilet paper']
    label_embeddings = np.zeros((len(fine_labels), 768))
    model.eval()
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            for idx, label in enumerate(fine_labels):
                # 获取 label 的嵌入向量
                embedding = model.get_learned_conditioning([label]
                                                           # , 0, opt
                                                           )
                # ipdb.set_trace()
                ...
                # ipdb.set_trace()
                # 保存嵌入向量到数组中
                label_embeddings[idx] = embedding.cpu().numpy()
    
    print(label_embeddings.shape)
    print(label_embeddings)    
    np.save('/home1/gaoheng/gh_workspace/GOLDEN_HOOP/data/outlier_data/token_embed_in1k.npy', label_embeddings)
