...
class MemoryEfficientCrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        q, k, v = map(
            lambda t: t.unsqueeze(3)
                .reshape(-1, t.shape[1], self.heads, self.dim_head)
                .permute(0, 2, 1, 3)
                .reshape(-1, t.shape[1], self.dim_head)
                .contiguous(),
            (q, k, v),
        )

        attn_weights = torch.einsum('bnd,bmd->bnm', q, k) * self.scale
        attn_weights = F.softmax(attn_weights, dim=-1)

        out = torch.einsum('bnm,bmd->bnd', attn_weights, v)
        out = (
            out.unsqueeze(0)
            .reshape(-1, self.heads, out.shape[1], self.dim_head)
            .permute(0, 2, 1, 3)
            .reshape(-1, out.shape[1], self.heads * self.dim_head)
        )
